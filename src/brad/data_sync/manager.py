...
class DataSyncManager:
    ...
    async def _sync_table(self, table: Table):
        ...
        try:
            ...
            await self._fetch_and_set_sync_bounds(aurora, ctx)
            ...
            await self._export_aurora_to_s3(aurora, ctx)

            # 3. Import into Redshift and run a merge.
            await self._import_s3_to_redshift(redshift, ctx)

            # 4. Run a merge in Athena.
            await self._merge_into_athena(athena, ctx)

            # 5. Commit new next_extract_seq values and remove extracted rows from the shadow table.
            await self._complete_sync(aurora, ctx)
    ...
    async def _export_aurora_to_s3(self, aurora, ctx: _SyncContext):
        ...
        await aurora.execute(extract_main)
        await aurora.execute(extract_shadow)

    async def _import_s3_to_redshift(self, redshift, ctx: _SyncContext):
        ...
        await redshift.execute(create_redshift_staging)
        logger.debug("Running on Redshift: %s", create_redshift_shadow_staging)
        await redshift.execute(create_redshift_shadow_staging)
        ...
        await redshift.execute(import_redshift_staging)
        logger.debug("Running on Redshift: %s", import_redshift_shadow_staging)
        await redshift.execute(import_redshift_shadow_staging)
        ...
        await redshift.execute(delete_using_redshift_staging)
        logger.debug("Running on Redshift: %s", delete_using_redshift_shadow_staging)
        await redshift.execute(delete_using_redshift_shadow_staging)
        ...
        await redshift.execute(insert_using_redshift_staging)

        # 3. e) Commit and clean up.
        await redshift.commit()
        await redshift.execute(
            "DROP TABLE {}".format(imported_staging_table_name(ctx.table))
        )
        await redshift.execute(
    ...
    async def _merge_into_athena(self, athena, ctx: _SyncContext):
        # a) Create staging tables.
        await athena.execute(
            "DROP TABLE IF EXISTS {}".format(imported_staging_table_name(ctx.table))
        )
        await athena.execute(
            "DROP TABLE IF EXISTS {}".format(
    ...
    async def _complete_sync(self, aurora, ctx: _SyncContext):
        ...
        if ctx.max_shadow_extract_seq != _MAX_SEQ:
            ...
            await aurora.execute(aurora_delete_shadow)
