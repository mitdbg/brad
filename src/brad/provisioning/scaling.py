...
class ProvisionScaling:
    # Create.
    def __init__(self, athena_catalog_name="bradcatalog0", redshift_cluster_name="brad-cluster0", aurora_cluster_name="brad-cluster0"):
    ...
    def can_boot_redshift(self, node_type="ra3.xlplus", node_count=2, window_minutes=10):
        athena_cost = self.get_athena_cost(window_minutes=window_minutes)
        redshift_cost = self.get_redshift_cost(node_type=node_type, node_count=node_count) * (window_minutes / 60)
        return athena_cost > redshift_cost


    # Check redshift utilization.
    def check_redshift_utilization(self, check_overused=True, utilization_type="cpu", utilization_threshold=90, window_minutes=10):
        end_time = datetime.datetime.utcnow()
        start_time = end_time - datetime.timedelta(minutes=window_minutes)
        period = 60*window_minutes
        # Return metrics.
        ...
        if utilization_type not in metric_name_map:
            raise ValueError("Invalid utilization type. Valid types are 'disk', 'cpu', and 'memory'.")
    ...
    def get_redshift_slow_percentage(self, running_time_threshold_secs=10, window_minutes=10):
    ...
    def check_aurora_utilization(self, check_overused=True, utilization_type="memory", utilization_threshold=80, window_minutes=10, node_type="db.r6g.large"):
        end_time = datetime.datetime.utcnow()
        start_time = end_time - datetime.timedelta(minutes=window_minutes)
        period = 60*window_minutes
        # Get metrics.
