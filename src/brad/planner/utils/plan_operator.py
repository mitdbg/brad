...
from brad.planner.utils.generate_workload import Aggregator, ExtendedAggregator, LogicalOperator
from brad.planner.utils.parse_filter import parse_filter, PredicateNode
from brad.planner.utils.postgres_utils import child_prod
    ...
    def add_filter(self, parse_tree, text=None):
        if parse_tree is not None:
            ...
            if existing_filter is not None:
                ...
                parse_tree = PredicateNode(None, [existing_filter, parse_tree], existing_filter)
                parse_tree.operator = LogicalOperator.AND

            if text is not None:
                self.plan_parameters.update(dict(filter_columns=parse_tree, filter_text=text))
            else:
