...
class PredicateNode:
    def __init__(self, text, children, existing_filter=None):
        self.raw_text = copy.deepcopy(existing_filter)
        self.text = text
        ...
        if all([k == "AND" for k in keywords]):
            ...
            for op_rep, op in repr_op:
                ...
                if split_str in self.text:
                    ...
                    is_having = column.lower() in agg_ops or (
                        len(self.children) == 1 and self.children[0].text in agg_ops
                    )
                    if is_having:
                        column = None
                        self.children = []
            ...
            if parse_baseline:
                if node_op in {Operator.IS_NULL, Operator.IS_NOT_NULL}:
                    ...
                    if "::text" in literal:
                        ...
                        literal = float(
                            literal.split("'::double precision")[0].strip("'")
                        )
...
def parse_recursively(filter_cond, offset, _class=PredicateNode):
    ...
    while True:
        ...
        if filter_cond[offset] == "(" and not escaped:
            child_node, offset = parse_recursively(
                filter_cond, offset + 1, _class=_class
            )
