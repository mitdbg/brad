...
class QueryLatencyCeiling(Trigger):
    ...
    async def should_replan(self) -> bool:
        ...
        if len(rel) >= self._sustained_epochs and (rel > self._latency_ceiling_s).all():
            ...
            logger.info(
                "Triggering replan because p90 query latency (%f) is above %f.",
                p90_lat_s,
        ...
        rel = future[FrontEndMetric.QueryLatencySecondP90.value]
        ...
        if (rel > self._latency_ceiling_s).all():
            p90_lat_s = rel.iloc[-1]
            logger.info(
                "Triggering replan because p90 query latency (%f) is forecasted to be above %f.",
                p90_lat_s,
                self._latency_ceiling_s,
            )
            return True
