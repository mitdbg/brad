...
def test_off_to_off() -> None:
    ...
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov, 1.0, 0.0, 0.0, 1.0, ctx
    )
...
def test_off_to_on() -> None:
    ...
    ctx = get_fixtures(redshift_cpu=[], redshift_prov=curr_prov)
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov, None, 2.0, 0.0, 1.0, ctx
    )
...
def test_on_to_on() -> None:
    ...
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov, 2.0, 0.0, 0.0, 1.0, ctx
    )
    assert cpu_util == pytest.approx(0.5)

    # Scale up, 0.5x movement (we stay conservative).
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov, 0.5, 0.0, 0.0, 1.0, ctx
    )
    ...
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, smaller_prov, 1.0, 0.0, 0.0, 1.0, ctx
    )
    assert cpu_util == pytest.approx(1.0)

    # Scale down, 2x movement.
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, smaller_prov, 2.0, 0.0, 0.0, 1.0, ctx
    )
    assert cpu_util == pytest.approx(1.0)

    # Scale down, 0.5x movement (stay conservative).
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, smaller_prov, 0.5, 0.0, 0.0, 1.0, ctx
    )
    ...
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov, None, 1.0, 0.0, 1.0, ctx
    )
    assert cpu_util == pytest.approx(0.5)
...
def test_on_to_on_with_skew() -> None:
    ...
    cpu_util = RedshiftProvisioningScore.predict_max_node_cpu_util(
        curr_prov, next_prov_instance, 1.0, 0.0, 0.0, 1.0, ctx
    )
    assert cpu_util == pytest.approx(0.5)
