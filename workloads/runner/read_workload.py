...
def read_workload_from_file(txn_query_dir: str, analytic_query_dir: str, num_txn_users: int,
    ...
    for i in range(num_analytic_users):
        ...
        query_file = os.path.join(analytic_query_dir, "adhoc_queries", f"analytic_user_{i + 1}.json")
        if os.path.exists(query_file):
            with open(query_file, "r") as f:
    ...
    for file in os.listdir(os.path.join(analytic_query_dir, "reporting_queries")):
        if file.endswith(".sql"):
            with open(os.path.join(analytic_query_dir, "reporting_queries", file), "r") as f:
                sql = f.read()
            reporting_queries.append(sql)
    return txn_users_queries, analytic_users_queries, reporting_queries


def make_imdb_workload(txn_query_dir: str, analytic_query_dir: str, num_txn_users: int,
                       num_analytic_users: int, reporting_time_window: Tuple[str, str],
                       num_days: int = 1,
                       start_time: str = "00:00:00",
                       auto_commit: bool = True, fast_forward_factor: float = 1.0) -> Workload:
    txn_users_queries, analytic_users_queries, reporting_queries = read_workload_from_file(txn_query_dir,
                                                                                           analytic_query_dir,
                                                                                           num_txn_users,
                                                                                           num_analytic_users)
    ...
    for sql in reporting_queries:
        query = Query(sql, Repeat.starting_at(
