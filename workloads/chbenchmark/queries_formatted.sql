...
-- Q5
-- Aurora PostgreSQL and Redshift
select	 n_name,
...
where	 c_id = o_c_id
	 ...
	 and ol_i_id = s_i_id
	 and mod((s_w_id * s_i_id),10000) = su_suppkey
	 and ascii(substring(c_state,1,1)) = su_nationkey -- problematic on Athena
	 and su_nationkey = n_nationkey
	 and n_regionkey = r_regionkey
	 and r_name = 'Europe'
	 and o_entry_d >= timestamp '2007-01-02 00:00:00.000000'
...
-- Q5
-- Athena
-- Simple hardcoded translation: Replace codepoint() with ascii().
select	 n_name,
	 sum(ol_amount) as revenue
from	 customer, orders, order_line, stock, supplier, nation, region
where	 c_id = o_c_id
	 ...
	 and ol_i_id = s_i_id
	 and mod((s_w_id * s_i_id),10000) = su_suppkey
	 and codepoint(cast(substring(c_state,1,1) as varchar(1))) = su_nationkey
	 and su_nationkey = n_nationkey
	 and n_regionkey = r_regionkey
	 and r_name = 'Europe'
	 and o_entry_d >= timestamp '2007-01-02 00:00:00.000000'
...
group by su_nationkey, substring(c_state,1,1), extract(year from o_entry_d)
order by su_nationkey, cust_nation, l_year


-- Q7 (Rewritten with CTE)
WITH inner_query AS (
  select	 su_nationkey as supp_nation,
    substring(c_state,1,1) as cust_nation,
    extract(year from o_entry_d) as l_year,
    ol_amount as revenue
  from	 supplier, stock, order_line, orders, customer, nation n1, nation n2
  where	 ol_supply_w_id = s_w_id
    and ol_i_id = s_i_id
    and mod((s_w_id * s_i_id), 10000) = su_suppkey
...
-- Q8
-- Requires ascii -> codepoint conversion for Athena
...
where	 i_id = s_i_id
	 ...
	 and c_w_id = o_w_id
	 and c_d_id = o_d_id
	 and n1.n_nationkey = ascii(cast(substring(c_state,1,1) as varchar(1)))
