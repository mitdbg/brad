...
class GenericConnection():
    ...
    def execute_sync(self, query: str) -> List[Tuple[Any, ...]]:
...
class TiDBTimingDriver(AbstractDriver):
    DEFAULT_CONFIG = {
    }

    def __init__(self, ddl: str) -> None:
        super().__init__("tidb", ddl)
        self._connection: Optional[GenericConnection] = None
    ...
    def makeDefaultConfig(self) -> Config:
        return TiDBTimingDriver.DEFAULT_CONFIG

    def loadConfig(self, config: Config) -> None:
        self._connection = GenericConnection(make_tidb_conn())
    ...
    def executeStart(self):
        ...
        measure_file_path = cond.in_output_dir("tidb_timing.csv")
    ...
    def doDelivery(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            self._connection.begin_sync()
            for d_id in range(1, constants.DISTRICTS_PER_WAREHOUSE + 1):
                r = self._connection.execute_sync(q["getNewOrder"].format(d_id, w_id))
                ...
                r = self._connection.execute_sync(q["getCId"].format(no_o_id, d_id, w_id))
                # r = self._cursor.fetchall_sync()
                c_id = r[0][0]

                r = self._connection.execute_sync(q["sumOLAmount"].format(no_o_id, d_id, w_id))
                # r = self._cursor.fetchall_sync()
                ol_total = decimal.Decimal(r[0][0])

                self._connection.execute_sync(
                    q["deleteNewOrder"].format(d_id, w_id, no_o_id)
                )
                updateOrders = q["updateOrders"].format(
                    o_carrier_id, no_o_id, d_id, w_id
                )
                self._connection.execute_sync(updateOrders)
                updateOrderLine = q["updateOrderLine"].format(
                    ol_delivery_d.strftime("%Y-%m-%d %H:%M:%S"), no_o_id, d_id, w_id
                )
                self._connection.execute_sync(updateOrderLine)
    ...
    def doNewOrderOriginal(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            for i in range(len(i_ids)):
                ...
                r = self._connection.execute_sync(q["getItemInfo"].format(i_ids[i]))[0]
            ...
            for item in items:
                if len(item) == 0:
                    self._connection.rollback_sync()
                    return
            ## FOR
            ...
            r = self._connection.execute_sync(get_warehouse)[0]
            # r = self._cursor.fetchone_sync()
            w_tax = r[0]
            wdc_warehouse_tax_rate = time.time()

            get_district = q["getDistrict"].format(d_id, w_id)
            r = self._connection.execute_sync(get_district)[0]
            # r = self._cursor.fetchone_sync()
            district_info = r
            ...
            r = self._connection.execute_sync(get_customer)[0]
            # r = self._cursor.fetchone_sync()
            customer_info = r
            ...
            self._connection.execute_sync(createOrder)
            self._connection.execute_sync(
            ...
            for i in range(len(i_ids)):
                ...
                r = self._connection.execute_sync(get_stock_info)[0]
                # r = self._cursor.fetchone_sync()
                ...
                self._connection.execute_sync(update_stock)
                ...
                self._connection.execute_sync(createOrderLine)
    ...
    def doNewOrder(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            self._connection.execute_sync(insert_order_line_query)
    ...
    def doOrderStatus(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            self._connection.begin_sync()
            if c_id != None:
                r = self._connection.execute_sync(
            ...
            r = self._connection.execute_sync(getLastOrder)
            # r = self._cursor.fetchall_sync()
            order = r[0]
            if order:
                r = self._connection.execute_sync(
    ...
    def doPayment(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            r = self._connection.execute_sync(q["getWarehouse"].format(w_id))
            # r = self._cursor.fetchall_sync()
            warehouse = r[0]

            r = self._connection.execute_sync(q["getDistrict"].format(w_id, d_id))
            # r = self._cursor.fetchall_sync()
            district = r[0]
            ...
            if customer[11] == constants.BAD_CREDIT:
                ...
                self._connection.execute_sync(updateCustomer)
            else:
                c_data = ""
                self._connection.execute_sync(
            ...
            self._connection.execute_sync(insertHistory)
    ...
    def doStockLevel(self, params: Dict[str, Any]) -> int:
        try:
            ...
            self._connection.begin_sync()
            r = self._connection.execute_sync(q["getOId"].format(w_id, d_id))
            # r = self._cursor.fetchall_sync()
            result = r[0]
            assert result
            o_id = result[0]
