    ...
    def doNewOrder(self, params: Dict[str, Any]) -> List[Tuple[Any, ...]]:
        try:
            ...
            for i in range(len(i_ids)):
                ...
                r = self._connection.execute_sync(q["getItemInfo"].format(i_ids[i]))
                # r = self._cursor.fetchall_sync()
                items.append(r[0])
            ...
            ## ----------------
            r = self._connection.execute_sync(q["getWarehouseTaxRate"].format(w_id))
            # r = self._cursor.fetchall_sync()
            w_tax = r[0][0]

            r = self._connection.execute_sync(q["getDistrict"].format(d_id, w_id))
            # r = self._cursor.fetchall_sync()
            district_info = r[0]
            d_tax = district_info[0]
            d_next_o_id = district_info[1]

            r = self._connection.execute_sync(q["getCustomer"].format(w_id, d_id, c_id))
            # r = self._cursor.fetchall_sync()
            customer_info = r[0]
