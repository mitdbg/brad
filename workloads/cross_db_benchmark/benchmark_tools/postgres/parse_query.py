...
from workloads.cross_db_benchmark.benchmark_tools.postgres.utils import (
    plan_statistics,
    getJoinConds,
    getFilters,
)
...
def create_join_node(
    ...
    if join_table_alias == [None, None]:
        ...
        if t1 not in scan_nodes or t2 not in scan_nodes:
            print(f"WARNING postres omitted one of your join condition {join_cond}")
            return None, None
...
def parse_query_from_plan(
    ...
    join_conds = getJoinConds(alias_dict, sql, table_id_mapping, column_id_mapping)
    if len(join_conds) == 0:
        return None
    ...
    parsed_query["PG_omit_join_cond"] = []
    has_timeout = False
    if is_explain_only:
    ...
    for join_cond in join_conds:
        ...
        if join_node is not None:
            ...
            parsed_query["PG_omit_join_cond"].append(join_cond)
    return parsed_query
...
def parse_plans_with_query_postgres(
    run_stats,
    min_runtime=100,
    max_runtime=30000,
    ...
    LOCAL_DSN="user=ubuntu password=postgres host=/tmp ",
    timeout_ms=None,
