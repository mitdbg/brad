...
def parse_queries_athena(run_stats, *args, **kwargs):
    # The logged query format is different depending on which "backend" we used
    # to record the data.
    if hasattr(run_stats.query_list[0], "status"):
        return parse_queries_athena_boto_format(run_stats, *args, **kwargs)
    else:
        return parse_queries_athena_classic_format(run_stats, *args, **kwargs)


def parse_queries_athena_boto_format(
    run_stats,
    aurora_run_stats,
    min_runtime=100,
    max_runtime=300000,
...
def parse_queries_athena_classic_format(
    run_stats,
    aurora_run_stats,
    min_runtime=100,
    max_runtime=300000,
