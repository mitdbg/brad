...
class PlanOperator(dict):
    ...
    def parse_lines(
        ...
        for l in self.plain_content[1:]:
            ...
            # probably in the future it would be easier to define a grammar and use a parser for this
                ...
                l = l.replace("Output: ", "")
                output_columns = self.parse_output_columns(l, is_brad)
            ...
            # Bitmap Scan Condition
            elif l.startswith("Recheck Cond: "):
                l = l.replace("Recheck Cond: ", "")
                parse_tree = parse_filter(l, parse_baseline=parse_baseline)
                self.add_filter(parse_tree)
    ...
    def parse_output_columns(self, l, is_brad=False):
        output_columns = []
        for col in l.split(", "):
            ...
            # for operators to change dates
                ...
                for filter_m in filter_columns_regex.finditer(col):
                    ...
                    if (
                        ...
                        continue
                    if is_brad and "brad_seq" in curr_col:
                        # ignore internal column of brad
                        continue
    ...
    def parse_columns_bottom_up(
        ...
        if table is not None:
            if table in table_id_mapping:
                ...
                assert (
                    self.plan_parameters["op_name"] == "Bitmap Index Scan"
                ), self.plan_parameters["op_name"]
    ...
    def parse_lines_recursively(
        ...
        # special case
        if self.plan_parameters["op_name"] == "Bitmap Heap Scan":
            self.children = []
        for c in self.children:
