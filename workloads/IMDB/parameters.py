...
class WorkloadParams(object):
    ...
    def Params(cls):
        p = Params()
        p.Define("num_days", 1, "number of days for simulation, the simulation pattern for these days will be the same."
        ...
        p.Define("total_num_txn_users", None, "total number of users who writes transaction queries.")
        p.Define("num_txn_queries_per_user", None, "number of transaction queries each user write per day.")
        p.Define("num_txn_queries_dist", None, "a list of length 24, representing number of txn queries for each hour.")
        p.Define("num_tuples_insert_per_user", None, "the average number of tuples to insert for each txn query.")
        p.Define("match_new_pk", False, "do we want the newly-inserted tuple's foreign key to match the old primary key"
                                        "or the newly-inserted primary key?")
        p.Define("table_insert_freq", None, "a dictionary indicating the insertion frequency for each table.")
        p.Define("txn_generation_ids_offset_file", None, "a log file for generating txn queries")

        # Parameters for analytical workload
        p.Define("analytic_query_dir", None, "directory for transaction queries.")
        p.Define("reporting_query_rt_interval", None, "a dictionary indicating how to select the reporting queries.")
        p.Define("reporting_time_window", None, "during which time of a day to run reporting queries.")
        p.Define("total_num_analytic_users", None, "total number of users who writes analytical queries.")
        p.Define("num_analytic_queries_per_user", None, "number of analytic queries each user write per day.")
        p.Define("num_analytic_queries_dist", None, "a list of length 24, representing number of analytic queries "
                                                    "for each hour.")
        p.Define("analytic_query_rt_interval", None, "the runtime interval of ad-hoc analytical queries")
...
class Default(WorkloadParams):
    ...
    def Params(self):
        ...
        p.num_txn_queries_dist = [0.003, 0.002, 0.001, 0.001, 0.002, 0.004, 0.005, 0.01, 0.05, 0.1, 0.2, 0.2,
                                  0.1, 0.2, 0.2, 0.2, 0.2, 0.1, 0.05, 0.01, 0.01, 0.003, 0.003, 0.003, 0.003]
        ...
        p.num_analytic_queries_dist = [0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.05, 0.1, 0.2, 0.2,
                                       0.1, 0.2, 0.2, 0.2, 0.2, 0.1, 0.05, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01]
