...
DEFINE_string(log_writes, "",
              "If set, will log the writes to a file for later analysis.");
...
int main(int argc, char* argv[]) {
            ...
            << std::endl;
  {
    ...
    store.ResetPhysIdSequence(c);
    store.UpdateMaxStats(c);
  }
  ...
  std::shared_ptr<Cirrus> cirrus =
      FLAGS_log_writes.empty()
          ? std::shared_ptr<Cirrus>(Cirrus::Open(config, strategy))
          : std::make_shared<CirrusWriteLogger>(FLAGS_log_writes);

  cirrus->EstablishThreadLocalConnections();
  if (StrategyUsesMaterializedView(strategy)) {
    cirrus->SetUpViews();
  }
  if (strategy != Strategy::kAllOnOne) {
    cirrus->SyncTableVersions();
  }

  std::vector<std::unique_ptr<MakeSale>> tclients;
  std::vector<std::unique_ptr<StockFeature>> aclients;

  // Start up the clients.
  std::cerr << "> Starting up and warming up aclients..." << std::endl;
  StockFeatureOptions aoptions;
  aoptions.num_warmup = FLAGS_warmup;
  for (uint32_t i = 0; i < FLAGS_aclients; ++i) {
    aclients.push_back(std::make_unique<StockFeature>(aoptions, cirrus, state));
  }
  ...
  toptions.max_s_datetime = store.MaxDatetime();
  toptions.max_i_id = store.MaxId();
  ...
  // Write out the stats
  {
    ...
    Stats::RunOnGlobal([&out](auto& stats) {
      ...
      out << "sales_notifs," << stats.GetSalesNotifications() << std::endl;
      out << "hot_inv_drops," << stats.GetHotInventoryDrops() << std::endl;
      out << "hot_sales_drops," << stats.GetHotSalesDrops() << std::endl;
      out << "view_maints," << stats.GetViewMaintInits() << std::endl;
      out << "read_with_pause," << stats.GetReadWithPause() << std::endl;
      out << "read_without_pause," << stats.GetReadWithoutPause() << std::endl;
      out << "manual_view_maints," << stats.GetManualViewMaints() << std::endl;
    });
  }
