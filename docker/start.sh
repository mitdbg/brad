...
# This script drops you into a BRAD development container. The container image
# must exist. If it does not exist, build it first using the setup.sh script.
#
...
if [ -z "$RUNNING" ] && [ -z "$EXITED" ]; then
  ...
  # NOTE: For stable measurements, it's important to ensure that the
